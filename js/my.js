...
var My = (function() {
    ...
    return function(div) {
        ...
        function openAll(anim, time, dom, type){
            ...
            if(!isShow){
                time && dom.setAttribute("style", "display:"+ type +"; -webkit-animation-duration:" + time + "ms;");
                anim = anim || "fadeAnim";
        ...
        function closeAll(anim, time, dom){
            isShow = (dom.style.display == 'block' || dom.style.display == 'inline-block') ? 1 : 0;

            if(isShow){
                time && dom.setAttribute("style", "-webkit-animation-duration:" + time + "ms;");
                anim = anim || "fadeAnim";
...
(function(my){
    ...
    function initAlert(){//生成弹窗信息层
        var c = document.createElement("div"),
            chtml = "";
        c.style.cssText = "box-shadow:0 -4px 0 0 #ddd; position:fixed;top:20%;left:50%;z-index:1400; margin-left:-130px;width:260px;font-size:14px; background-color:#fff;border-radius: 5px;";
        c.className = "alert";
    ...
    function initMask(){ //创建遮罩
        var c = document.createElement("div");
        c.style.cssText = "position:fixed;left:0;top:0;z-index:99;width:100%;height:100%;background-color:rgba(0,0,0,.6);display:none;";
        document.body.appendChild(c);
        c.addEventListener('touchmove', stopProp); //阻止滚动
        return _id(c);
    }
...
(function(my){
    ...
    function initTip() { //生成提示信息
        var c = document.createElement("div"),
            ...
            chtml = "" ;
        c.style.cssText = "display:none;position: fixed;top: 30%;margin: 0 auto;z-index: 1300;width: 100%;overflow: hidden;text-align: center;";
        chtml = "<div style = 'color: #fff;font-size: 16px;line-height: 22px;padding: 10px 15px;margin: 0 auto;display: inline-block;border-radius: 5px;max-width: 200px;background: rgba(0, 0, 0, 0.6);'></div>";
        c.innerHTML = chtml;
...
(function(my){
    ...
    function initFollow(){ //生成关注引导层
        ...
        if(!my.FollowImg){
			...
			c.style.cssText = "position:fixed;z-index:1100;width:100%;height:100%;top:0;bottom:0;right:0;left:0;background-color:rgba(0,0,0,.6);";
